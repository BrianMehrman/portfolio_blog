...
$ echo 'Here is some text to CanvasToImage.' | ./CanvasToImage.rb -m text/plain
data:text/plain,Here%20is%20some%20text%20to%20CanvasToImage.%0A

$ echo 'Here is some text to CanvasToImage.' | ./CanvasToImage.rb -m application/octet-stream
data:application/octet-stream;base64,SGVyZSBpcyBzb21lIHRleHQgdG8gZGF0YWZ5Lgo=

CanvasToImage will also fetch and encode from an HTTP URL: ./CanvasToImage.rb -u http://...
...
module CanvasToImage
  ...
  def CanvasToImage::make_data_uri(content, content_type)
  ...
  def CanvasToImage::decode_data_uri(uri)
  ...
  def CanvasToImage::page_to_data_uri(uri)
    uri = URI.parse(uri) unless uri.kind_of? URI
  ...
  def CanvasToImage::encode_page(body, content_type, base_uri)
    case content_type
    when 'text/html'
  ...
  def CanvasToImage::cleanup_uri(base, link)
  ...
  def CanvasToImage::fetch_page(uri)
    response = nil
    begin
...
if __FILE__ == $0 # don't polute the namespace with help
  ...
  def help(exitcode = 0)
    $stderr.puts "CanvasToImage.rb, an RFC2397 data: uri utility"
  ...
  if url.empty?
    ...
    puts CanvasToImage.make_data_uri(content, content_type)
  else
    begin
      page, = CanvasToImage.decode_data_uri(CanvasToImage.page_to_data_uri(url))
      puts page
    rescue => err
